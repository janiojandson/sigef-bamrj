...
class DashboardController {
    ...
    public function index() {
        ...
        if (in_array($role, ['OMAP', 'Setor_BAMRJ'])) {
            ...
            $stmt = $db->prepare($sql); $stmt->execute([$origem, $username, $ano]);
            $lotes = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } elseif ($role === 'Operador') {
            $sql = "SELECT DISTINCT l.*, i.status_atual as status_inbox FROM de_lotes l JOIN de_itens i ON l.id = i.lote_id WHERE i.status_atual NOT IN ('ARQUIVADO', 'CANCELADO_PELA_ORIGEM', 'AGUARDANDO_RECEBIMENTO_PROTOCOLO') AND EXTRACT(YEAR FROM l.criado_em) = ? ORDER BY l.criado_em DESC";
            $stmt = $db->prepare($sql); $stmt->execute([$ano]);
            $lotes = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } elseif ($role === 'Protocolo') {
            $sql = "SELECT DISTINCT l.*, i.status_atual as status_inbox FROM de_lotes l JOIN de_itens i ON l.id = i.lote_id WHERE i.status_atual = 'AGUARDANDO_RECEBIMENTO_PROTOCOLO' AND EXTRACT(YEAR FROM l.criado_em) = ? ORDER BY l.criado_em ASC";
            $stmt = $db->prepare($sql); $stmt->execute([$ano]);
            $lotes = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            if (!empty($fases_permissao)) {
                $in = str_repeat('?,', count($fases_permissao) - 1) . '?';
                $sql = "SELECT DISTINCT l.*, i.status_atual as status_inbox FROM de_lotes l JOIN de_itens i ON l.id = i.lote_id WHERE i.status_atual IN ($in) AND EXTRACT(YEAR FROM l.criado_em) = ? ORDER BY l.criado_em DESC";
                $stmt = $db->prepare($sql); $stmt->execute(array_merge($fases_permissao, [$ano]));
                $lotes = $stmt->fetchAll(PDO::FETCH_ASSOC);
            }
        }
