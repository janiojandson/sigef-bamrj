            ...
            try {
                ...
                if (in_array($tipo_acao, ['inserir_np', 'inserir_lf', 'atender_fin', 'inserir_op'])) {
                    ...
                    if (empty($itens_selecionados)) { 
                        echo "<script>alert('Atenção: Você deve marcar as caixinhas na tabela para realizar esta ação!'); history.back();</script>"; 
                        exit(); 
                    }
                    ...
                    foreach ($itens_selecionados as $item_id) {
                        ...
                        // 🛡️ CORREÇÃO CRÍTICA: Uso de Prepared Statement para não quebrar a transação no loop
                        ...
                        $db->prepare("INSERT INTO de_eventos (item_id, usuario_nip, perfil_atuante, acao, fase_anterior, fase_nova, justificativa) VALUES (?, ?, ?, ?, ?, ?, ?)")
                           ->execute([$item_id, $usuario, $perfil, $acao_log, $fase_anterior, $novo_status, $observacao_atual]);
                    }
                } 
                // 📌 AÇÕES INDIVIDUAIS
                else {
