...
class DEController {
    ...
    public function store() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $cpfs = $_POST['cpf_cnpj'] ?? []; $docs = $_POST['num_doc_fiscal'] ?? []; $valores = $_POST['valor_total'] ?? []; $pas = $_POST['pa_numero'] ?? []; $prioridades = $_POST['prioridade_flag'] ?? [];
            $usuario = $_SESSION['username']; $perfil = $_SESSION['role'];
            ...
            try {
                ...
                for ($i = 0; $i < count($cpfs); $i++) {
                    $cpf_cnpj = preg_replace('/\D/', '', $cpfs[$i]); $num_doc = trim($docs[$i]); $valor_total = str_replace(['.', ','], ['', '.'], $valores[$i]);
            ...
            } catch (Exception $e) { $db->rollBack(); die("<div style='padding:20px;'><h1>⚠️ Falha</h1><p>" . $e->getMessage() . "</p></div>"); }
        }
    }
    ...
    public function reenviar() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $db = Database::getConnection();
            $item_id = $_POST['item_id'] ?? 0; $lote_id = $_POST['lote_id'] ?? 0; $observacao = trim($_POST['observacao'] ?? 'Corrigido e reenviado.');
            $novo_doc = trim($_POST['num_doc'] ?? ''); $novo_valor = str_replace(['.', ','], ['', '.'], $_POST['valor'] ?? '0');
            ...
            try {
                $db->beginTransaction();
                $db->prepare("UPDATE de_itens SET status_atual = 'AGUARDANDO_RECEBIMENTO_PROTOCOLO', observacao_atual = ?, num_documento_fiscal = ?, valor_total = ? WHERE id = ?")->execute([$obs_formatada, $novo_doc, $novo_valor, $item_id]);
                $db->prepare("INSERT INTO de_eventos (item_id, usuario_nip, perfil_atuante, acao, fase_nova, justificativa) VALUES (?, ?, ?, 'REENVIAR_ORIGEM', 'AGUARDANDO_RECEBIMENTO_PROTOCOLO', ?)")->execute([$item_id, $usuario, $perfil, "Doc ajustado para $novo_doc / R$ $novo_valor. Obs: $observacao"]);
                $db->commit(); header("Location: /de/acompanhar?id=" . $lote_id); exit();
    ...
    public function excluirItem() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $db = Database::getConnection();
            $item_id = $_POST['item_id'] ?? 0; $lote_id = $_POST['lote_id'] ?? 0; $motivo = trim($_POST['motivo_cancelamento'] ?? 'Cancelamento solicitado.');
            $usuario = $_SESSION['username']; $perfil = $_SESSION['role'];
